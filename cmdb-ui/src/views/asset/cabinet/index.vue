<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['asset:cabinet:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['asset:cabinet:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="cabinetList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="机柜布局" align="center" prop="cabinetLayout" />
...
:value="dict.value"
        ...
        <el-form-item label="机柜布局" prop="cabinetLayout">
          <el-input v-model="form.cabinetLayout" type="textarea" placeholder="请输入内容" />
        </el-form-item>
      </el-form>
...
<script>
import { listCabinet, getCabinet, delCabinet, addCabinet, updateCabinet } from "@/api/asset/cabinet";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getCabinet(number).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除机柜管理编号为"' + numbers + '"的数据项？').then(function() {
        return delCabinet(numbers);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('asset/cabinet/export', {
        ...this.queryParams
